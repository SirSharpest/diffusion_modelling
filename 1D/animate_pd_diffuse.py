...
def make_double_animation(cell_states, chem, pd,  N, Nx, ts, average, savefigs=True):
    fig, axes = plt.subplots(2, figsize=(15, 5))
    x_labels_locations = np.linspace(0, N if average else N*Nx, num=N+1)

    if average:
        cells = cell_states[chem][pd][0]
    else:
        cells = cell_states[chem][pd][0].ravel()

    x_labels = ['C{0}'.format(n) for n in range(N+1)]
    axes[0].plot(cells)
    axes[0].grid(False)
    axes[0].set_ylim(1e-12, 10)
    axes[0].set_yscale('log')
    axes[0].set_xlim(0, N if average else N*Nx)
    ...
    fig.suptitle(
        'Chem:{0} | PD:{1} | H:{2} M:{3} S:{4}'.format(chem, pd, 0, 0, 0))
    ...
    def animate(i):
        ...
        axes[0].grid(False)
        if average:
            cells = cell_states[chem][pd][i]
        else:
            cells = cell_states[chem][pd][i].ravel()

        for j in range(0, N if average else Nx*N, 1 if average else Nx):
            axes[0].axvline(j-0.5 if average else j, color='red')
        ...
        fig.suptitle(
            'Chem:{0} | PD:{1} | H:{2} M:{3} S:{4}'.format(chem, pd, i//(60*60), (i//60) % 60, i % 60))

    anim = animation.FuncAnimation(
        fig, animate, frames=np.linspace(1, ts, dtype=int, num=200),
        blit=False, interval=200)
    # plt.tight_layout()
    savename = '{0}_{1}_{2}'.format(chem, pd, ts).replace('.', '')
    print(savename)

    anim.save('./media/{0}/{1}-extended.mp4'.format(chem, savename))

    if savefigs:
        for i in np.linspace(1, ts, dtype=int, num=int(ts//(60*60))):
            animate(i)
            fig.savefig(
                './media/{0}/stills/{1}-{2}-extended.png'.format(chem, savename, i))
